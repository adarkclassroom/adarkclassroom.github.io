...
var Room = {
	...
	onArrival: function(transition_diff) {
		Room.setTitle();
		if(Room.changed) {
			Notifications.notify(Room, _("the fire is.", Room.FireEnum.fromInt($SM.get('game.fire.value')).text));
			Notifications.notify(Room, _("{0}.", Room.TempEnum.fromInt($SM.get('game.temperature.value')).text));
			Room.changed = false;
		}
	...
	setTitle: function() {
		var title = $SM.get('game.fire.value') < 2 ? _("A Dark Classroom") : _("A Lit Classroom");
	...
	updateBuilderState: function() {
		...
		if(lBuilder === 0) {
			Notifications.notify(Room, _("a stranger stumbles through the door and collapses in the corner"));
			...
			switch(lBuilder) {
			case 1:
				msg = _("the stranger shivers, and mumbles unintelligibly under her breath.");
				break;
			case 2:
